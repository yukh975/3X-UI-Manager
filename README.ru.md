...
Паритет функций с [Android-приложением](https://github.com/MHSanaei/3x-ui) (полный список — в README ветки `main`): подключение по **API-токену**, **несколько панелей (профилей)** с переключателем, живой **дашборд**, **входящие** (вкл/правка), **клиенты** (поиск, правка, QR/подписка), **узлы** (в т.ч. mTLS), структурный редактор **конфига Xray**, **бэкап/восстановление**, **администрирование панели** и опциональная **блокировка приложения** (код-пароль + биометрия), защищающая только залогиненный интерфейс. **Проверка обновлений в приложении** (при запуске + кнопка «Проверить обновления») уведомляет о новом GitLab-релизе и открывает страницу релиза — iOS не может само установить неподписанную сборку, поэтому загрузка и сайдлоад вручную.
