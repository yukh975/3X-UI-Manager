...
**Релизы:** последний **выпущенный** — **v0.3.14** («первый стабильный» был
v0.3.11). В v0.3.14 вошли: трафик за месяц, обновление гео-баз, совместимость с
панелью v3.3.0 (эндпоинты под `/panel/api`) и фикс «онлайн по серверам» (v0.3.13
свёрнут в неё).

**Рабочий цикл (с этого момента):** код → локальная сборка (`./gradlew
:app:assembleDebug`) → **обязательная проверка в эмуляторе на этом компе** → и
только после отладки тег `vX.Y.Z` → сборка в GitLab с публикацией релиза.
